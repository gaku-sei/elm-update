...
versionParser =
  (fromString <<< fromCharArray <$> many digit) `sepBy1` (string ".")
    <#> fromFoldable
    >>= case _ of
        [ Just major, Just minor, Just patch ] -> pure $ Version { major, minor, patch }
        _ -> fail "Version format is not valid"
...
instance readVersion :: Read Version where
  read = hush <<< flip runParser versionParser
...
newtype DependencyMap
  = DependencyMap (Map String (Maybe Version))
...
newtype NewerDependencyMap
  = NewerDependencyMap (Map String (Array Version))
...
instance showNewerDependencyMap :: Show NewerDependencyMap where
  show (NewerDependencyMap dependencyMap) = dependencyMap # foldrWithIndex format [] # joinWith "\n"
    where
    format :: String -> Array Version -> Array String -> Array String
    format k = flip snoc <<< (maybe "" ((<>) (k <> ": ") <<< show) <<< maximum)
